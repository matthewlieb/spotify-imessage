/**
 * SpotifiMessage - Simple, Fast, Secure
 * 
 * Flow:
...
function App() {
  ...
  // Intentionally run once on mount for OAuth callback; no stable deps needed
  ...
  const processPastedText = async () => {
    if (!pastedText.trim()) {
      setError('Please paste some message content first');
      return;
    }
    ...
    try {
      // Extract Spotify links from pasted text
      const spotifyPattern = /(?:open|www|embed)\.spotify\.com\/(track|album|playlist)\/([A-Za-z0-9]{22})|spotify:(track|album|playlist):([A-Za-z0-9]{22})/g;
      const matches = [...pastedText.matchAll(spotifyPattern)];
      const trackIds = [...new Set(matches.map(m => m[2] || m[4]).filter(Boolean))];

      if (trackIds.length === 0) {
        setError('No Spotify links found in the pasted text');
        setScanning(false);
        return;
      }

      // Create a virtual chat from pasted content
      const virtualChat = {
      ...
      setSelectedChat(virtualChat.name);
      
      // Load track details
      await loadTracksFromIds(trackIds);
      setPastedText('');
  ...
  const loadTracksFromIds = async (trackIds) => {
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="glass-card p-6 mb-6">
          <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
            <div>
              <h2 className="text-2xl font-semibold mb-1">1. Get Your Messages</h2>
              <p className="text-sm text-gray-400">
                {isMobile 
                  ? 'Paste messages with Spotify links below' 
                  : 'Scan your iMessage chats or paste messages'}
              </p>
            </div>
            {!isMobile && (
              <button
            ...
            <div className="mt-4 space-y-4">
              <div>
                <label className="block text-sm font-medium mb-2 text-gray-300">
                  Paste messages with Spotify links:
                </label>
                <textarea
                  value={pastedText}
                  onChange={(e) => setPastedText(e.target.value)}
                  placeholder="Paste your messages here... (e.g., Check out this song: https://open.spotify.com/track/...)"
            ...
            <div className="mt-4">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const chat = chats.find((c) => c.name === value);
                  if (chat?.trackIds?.length) {
                    setSelectedChat(value);
                    loadTracksFromIds(chat.trackIds);
                  } else {
                    loadTracks(value);
                  }
                }}
